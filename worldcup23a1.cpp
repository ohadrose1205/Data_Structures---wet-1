...
StatusType world_cup_t::remove_team(int teamId)
{
    ...
    if(!teamToDelete || !teamToDelete->data().empty()){
        return StatusType::FAILURE;
    }
...
StatusType world_cup_t::remove_player(int playerId){
    ...
    Player saveForAfterDelete = foundPlayer->data();
    StatusType checker1 = remove_playerAUX(foundPlayer->data());
    if(checker1 == StatusType::SUCCESS) {
        if (saveForAfterDelete.getTeam()->isTeamValid()) {
            m_validTeamsTree.remove(saveForAfterDelete.getTeam()->getTeamId());
        }
    }
    return checker1;
}
...
StatusType world_cup_t::unite_teams(int teamId1, int teamId2, int newTeamId) {
    ...
    if (!teamPair1 || !teamPair2 || (newTeamId != teamId1 && newTeamId != teamId2 && m_teamsTree.find(newTeamId))){
        return StatusType::FAILURE;
    }
    ...
    if(newTeam){
        ...
        if (checker1 == StatusType::SUCCESS) {
            if(newTeam->isTeamValid()){
                Team& teamInTree = m_teamsTree.find(newTeamId)->data();
                teamInTree.updateAllPlayerTeamTag();
                StatusType checker2 = m_validTeamsTree.insert(&teamInTree, newTeamId);
                if(checker2 == StatusType::SUCCESS){
                    delete newTeam;
                    newTeam = nullptr;
                    return StatusType::SUCCESS;
...
output_t<int> world_cup_t::get_top_scorer(int teamId)
{
    ...
    if(teamId > 0) {
        Pair<Team,int>* teamPair = m_teamsTree.find(teamId);
        if(!teamPair || teamPair->data().empty())
            return StatusType::FAILURE;
...
StatusType world_cup_t::get_all_players(int teamId, int *const output)
{
    ...
    Pair<Team,int>* teamPair = m_teamsTree.find(teamId);
    if((teamId < 0 && m_playersById.empty()) || (teamId > 0 && (!teamPair || teamPair->data().empty())))
        return StatusType::FAILURE;
    if(teamId < 0 ){
        Pair<Player*, Player>** arrOfPlayers = nullptr;
        m_playersByGoals.inOrderScanToArray(arrOfPlayers);
        if(!arrOfPlayers)
            return StatusType::ALLOCATION_ERROR;
...
output_t<int> world_cup_t::get_closest_player(int playerId, int teamId)
{
    ...
    Pair<Team,int>* teamPair = m_teamsTree.find(teamId);
    if(!teamPair || m_playersById.size() <= 1) {
        return StatusType::FAILURE;
    }
...
output_t<int> world_cup_t::knockout_winner(int minTeamId, int maxTeamId)
{
    ...
    Pair<Team*, int> **arrayOfLegalTeam = nullptr;
    Pair<int, int> **teamValuesAndId = nullptr;
    int sizeArr = 0;
    if(!m_validTeamsTree.inOrderScanToArrayFromTo(arrayOfLegalTeam, sizeArr, minTeamId, maxTeamId))
        return StatusType::FAILURE;
    teamValuesAndId = new Pair<int, int>*[sizeArr]();
    for(int i = 0; i < sizeArr; i++){ //good complexity cause sizeArr equal to r valid teams
        teamValuesAndId[i] = new Pair<int, int>(arrayOfLegalTeam[i]->data()->teamValue(), arrayOfLegalTeam[i]->key());
    }
    delete[]arrayOfLegalTeam;
    int roundSize = sizeArr;
    int winnerKey, winnerValue, nextRoundSize;
    while(roundSize > 1) {
        nextRoundSize = 0;
        for (int j = 0; j < roundSize; j += 2) {
            ...
            winnerValue = teamValuesAndId[j]->data() + teamValuesAndId[j + 1]->data() + 3;
            delete teamValuesAndId[j];
            delete teamValuesAndId[j+1];
            teamValuesAndId[nextRoundSize] = new Pair<int, int>(winnerValue, winnerKey);
            nextRoundSize++;
        }
        roundSize = nextRoundSize;
    }
    int winner = teamValuesAndId[0]->key();
    delete teamValuesAndId[0];
